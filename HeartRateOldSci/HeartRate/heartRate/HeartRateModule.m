...
@property(nonatomic, weak) UIView* cameraParentDisplay;
@end
...
//        [alertView show];
    ...
    if (_cameraParentDisplay != nil) {
        _captureLayer = [AVCaptureVideoPreviewLayer layerWithSession: session];
        _captureLayer.frame = _cameraParentDisplay.bounds;
        _captureLayer.videoGravity = AVLayerVideoGravityResizeAspectFill;
        [_cameraParentDisplay.layer addSublayer: _captureLayer];
    }
...
-(void)setCameraDisplayView:(UIView*)pView{
    _cameraParentDisplay = pView;
}
-(void)setCameraDisplay:(BOOL)bDisplay{
    if (bDisplay) {
        [_cameraParentDisplay.layer addSublayer:_captureLayer];
    }else{
...
//     NSLog(@"h:%f",h);
    ...
    if(s>0.5 && v>0.2) {
        ...
        [self handleLowValue:0 ctx:context];
    }
     //[self handleLowValue:lowPassValue ctx:context];
    //    NSLog(@"addedLowPassValue:%f",lowPassValue);
